...
def run(context):
    ...
    try:
       ...
       # === Trim bottom with rectangle sketch on XZ Plane ===
        sketches = rootComp.sketches
        xzPlane = rootComp.xZConstructionPlane
        sketch_trim_down = sketches.add(xzPlane)

        lines_trim = sketch_trim_down.sketchCurves.sketchLines
        pt1 = adsk.core.Point3D.create(0, 0, -5)
        pt2 = adsk.core.Point3D.create(0, 0, 5)
        pt3 = adsk.core.Point3D.create(0, +4, 5)
        pt4 = adsk.core.Point3D.create(0, +4, -5)

        lines_trim.addByTwoPoints(pt1, pt2)
        lines_trim.addByTwoPoints(pt2, pt3)
        lines_trim.addByTwoPoints(pt3, pt4)
        lines_trim.addByTwoPoints(pt4, pt1)

        prof_trim_down = sketch_trim_down.profiles.item(0)

        extrudes = rootComp.features.extrudeFeatures
        extInput2 = extrudes.createInput(prof_trim_down, adsk.fusion.FeatureOperations.CutFeatureOperation)
        distance2 = adsk.core.ValueInput.createByReal(10)
        extInput2.setDistanceExtent(True, distance2)
        extInput2.isSolid = True
        extrudes.add(extInput2)

        # === Trim bottom with rectangle sketch on XZ Plane ===
        sketch_trim_up = sketches.add(xzPlane)

        lines_trim = sketch_trim_up.sketchCurves.sketchLines
        pt1 = adsk.core.Point3D.create( 0 , -plane_offset*num_repetition*2 , -5 )
        pt2 = adsk.core.Point3D.create( 0 , -plane_offset*num_repetition*2 , 5 )
        pt3 = adsk.core.Point3D.create( 0 , -plane_offset*num_repetition*2 - 3 , 5 )
        pt4 = adsk.core.Point3D.create( 0 , -plane_offset*num_repetition*2 - 3 , -5 )
        ...
        prof_trim_up = sketch_trim_up.profiles.item(0)

        extInput2 = extrudes.createInput(prof_trim_up, adsk.fusion.FeatureOperations.CutFeatureOperation)
        distance2 = adsk.core.ValueInput.createByReal(10)
        extInput2.setDistanceExtent(True, distance2)
        extInput2.isSolid = True
        extrudes.add(extInput2)
        ...
        # Get the first valid solid body as the target
        targetBody = None
        ...
        if not targetBody:
            ui.messageBox("No valid solid body found for combination.")
            return

        # Create tool body collection with all other valid solids
        toolBodies = adsk.core.ObjectCollection.create()
        for i in range(rootComp.bRepBodies.count):
            b = rootComp.bRepBodies.item(i)
            if b != targetBody and b.isSolid and b.isVisible:
                toolBodies.add(b)

        # Combine only if there is at least one tool body
        if toolBodies.count == 0:
            ui.messageBox("No tool bodies available for combination.")
            return
        ...
        # Get the body/component to export
        product = app.activeProduct
        rootComp = product.rootComponent

        # # Define the STEP export options
        # stepFileName = 'CombinedModel.stp'  # Change if desired
        # stepFilePath = app.userDocumentsFolder.path + '/' + stepFileName

        stepFilePath = '/Users/hugopenichou/Desktop/SAVE_MY_ASS/FEA/GET_STIFNESS/origami_living_hinge.stp'

        stepOptions = exportMgr.createSTEPExportOptions(stepFilePath, rootComp)
        stepOptions.meshRefinement = adsk.fusion.MeshRefinementSettings.MeshRefinementLow

        # Export the file
        exportMgr.execute(stepOptions)

        ui.messageBox(f"Exported STEP file to:\n{stepFilePath}")
