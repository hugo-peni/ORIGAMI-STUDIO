...
[Node, Panel] = ReadOBJ('yoshimura_pattern.obj');
...
%% Set up boundary conditions for yoshimura

indsupp = find(Node(:,3)<0.01);
nsupp = numel(indsupp);
YoshimuraSupport = [          indsupp(1), 1, 1, 1;
                  indsupp(2), 1, 1, 1;
        indsupp(3:end), zeros(nsupp-2,1)+1, zeros(nsupp-2,1)+1, ones(nsupp-2,1);];
Yoshimura_m = size(Node,1);
indp = find( abs( Node(:,3) - max(Node(:,3 ) ) ) < 1e-5 ) ; %indp = indp(3);
npp = numel(indp) ;

% percent = 40 ; 
% Load = max(Node(:,3 ) ) * percent / 100 ;
Load = 4.4 ;
YoshimuraLoad = [ indp , 0*ones(npp,1) , 0*ones(npp,1) , -Load*ones(npp,1) ; ] ;
...
%% Perform yoshimura analysis

% Assemble input data
[truss, angles, AnalyInputOpt] = PrepareData(Node,Panel,YoshimuraSupport,YoshimuraLoad,AnalyInputOpt) ;
